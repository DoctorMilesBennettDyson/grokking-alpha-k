"""
train_v3.py -- Debug variant for phase8_debug.py.

Extension over train_v2.py:
  - n_layers parameter exposed (was hardcoded to 2)
  - All other behavior identical to train_v2.py

Used only by phase8_debug.py for diagnostic runs.
"""
...
def train_single_run_v3(
    ...
    optimizer='adamw',
    init_scheme='xavier',
    n_layers=2,  # NEW: configurable depth of transformer encoder
):
    torch.manual_seed(seed)
    np.random.seed(seed)

    if run_name is None:
        run_name = (f"p{p}_d{depth}_nl{n_layers}_s{seed}_wd{weight_decay}"
                    f"_opt{optimizer}_lr{lr}")
    ...
    print(f"  Model params: {n_params:,}  |  optim={optimizer}  init={init_scheme}  "
          f"n_layers={n_layers}  warmup={warmup_steps}  lr={lr}")
    ...
    for step in range(steps):
        ...
        if step % eval_every == 0:
            ...
            if step % (eval_every * 50) == 0:
                ...
                print(f"  Step {step:>7d} | tr={tr_acc:.4f} te={te_acc:.4f} "
                      f"loss={tr_loss:.4f} | {grok_str} | {elapsed:.0f}s")
            if not (tr_loss == tr_loss) or tr_loss > 100:
    ...
    result = {
        'run_name': run_name, 'p': p, 'depth': depth, 'seed': seed,
        'dropout': dropout, 'label_smoothing': label_smoothing,
        'weight_decay': weight_decay, 'lr': lr, 'batch_size': batch_size,
        'optimizer': optimizer, 'init_scheme': init_scheme, 'n_layers': n_layers,
        'warmup_steps': warmup_steps, 'steps': steps, 'n_params': n_params,
        'n_train': len(X_tr), 'n_test': len(X_te),
        't_grok': t_grok, 't_train_saturated': t_train_saturated,
        'final_test_acc': final_test_acc, 'total_time_sec': total_time,
        'csv_path': csv_path, 'diverged': diverged,
    }
